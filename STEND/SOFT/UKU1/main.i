...
const char sm_mont[13][4]={"   ","ÿíâ","ôåâ","ìàð","àïð","ìàé","èþí","èþë","àâã","ñåí","îêò","íîÿ","äåê"};
const char sm_dow[8][3]={"ÏÍ","ÂÒ","ÑÐ","×Ò","ÏÒ","ÑÁ","ÂÑ"}; 
...
if(ips_bat_av_stat)
	{
	sub_ptrs[i++]=	"  Àâàðèÿ áàòàðåè    ";
	sub_cnt_max++;	
	}

if(uout_av)
	{
	sub_ptrs[i++]=	"   Àâàðèÿ Uâûõ!!!   ";
	sub_cnt_max++;	
	}
...
if(overloadAvar)
	{
	sub_ptrs[i++]=	" Ïåðåãðóçêà ïî òîêó!";
	sub_cnt_max++;	
	}

if(sysTAvar)
	{
	sub_ptrs[i++]=	" Ïåðåãðåâ ñèñòåìû!  ";
	sub_cnt_max++;	
	}

if(uOutAvar==1)
	{
	sub_ptrs[i++]=	"   Çàâûøåíî Uâûõ.!  ";
	sub_cnt_max++;	
	}

if(uOutAvar==2)
	{
	sub_ptrs[i++]=	"   Çàíèæåíî Uâûõ.!  ";
	sub_cnt_max++;	
	}

if(uInAvar==1)
	{
	sub_ptrs[i++]=	"   Çàâûøåíî Uâõ.!   ";
	sub_cnt_max++;	
	}

if(uInAvar==2)
	{
	sub_ptrs[i++]=	"   Çàíèæåíî Uâõ.!   ";
	sub_cnt_max++;	
	}
...
if(sub_cnt1>=20)
	...
	ptrs[0]	=			" 0%:0^:0& 0</>  /0{ ";
	ptrs[1]=			"                    ";	
	ptrs[2]=			" Ïîëíàÿ ïðîâåðêà    ";
    ptrs[3]=			" Êîíòðîëü îïòðîíîâ  ";
	ptrs[4]=			" Óñòàíîâêà âðåìåíè  ";
	ptrs[5]=			" Äàò÷èê âîçäóõà     ";
	ptrs[6]=			" Äàò÷èê âîäû        ";	
	ptrs[7]=			" Ïðîáíàÿ ñìñ        ";	
	ptrs[8]=			" Çàâîäñêèå óñòàíîâêè";						
	...
	if(optr_stat[2]==osON)	sub_bgnd("ÂÊË.",'!',-3);
	else 					sub_bgnd("ÂÛÊË.",'!',-4);
	if(optr_stat[1]==osON)	sub_bgnd("ÂÊË.",'%',-3);
	else 					sub_bgnd("ÂÛÊË.",'%',-4);
	if(optr_stat[0]==osON)	sub_bgnd("ÂÊË.",'(',-3);
	else 					sub_bgnd("ÂÛÊË.",'(',-4);
	...
	if(av_j_si_max==0)
		...
		else bgnd_par(	"ÂÛÐÀÂÍÈÂÀÞÙÈÅ ÇÀÐßÄÛ",
					"     ÁÀÒÀÐÅÈ N!     ",
					" (                  ",
					" [                  "); 
		pointer_set(2);			        
	...
	{
	bgnd_par("  Ââåäèòå  ïàðîëü   ",sm_,sm_,sm_);
	...
	{
	if(BAT_IS_ON[a_ind . s_i1]==bisON)ptrs[0]="Äëÿ âûâåäåíèÿ áàò.-è";
	else  ptrs[0]="Äëÿ ââåäåíèÿ áàòàðåè";
	bgnd_par(ptrs[0],"  íàáåðèòå ïàðîëü   ",sm_,sm_);
	...
	ptrs[1]=		" Âðåìÿ è äàòà       ";
     ptrs[2]=		" Ñòðóêòóðà          ";
	ptrs[3]=		" Êëèìàòêîíòðîëü     ";
	ptrs[4]=		" Âûõîä              ";
     ptrs[5]=		" Ìíåìîíèêà         y";
	ptrs[6]=		" Çâ.ñèãí.   (       ";
	ptrs[7]=		" Îòêëþ÷åíèå ñèãíàëà ";
	ptrs[8]=		"  àâàðèè    )       ";
	ptrs[9]=		" ÀÏÂ èñòî÷íèêîâ     ";
	ptrs[10]=		" Ïàðàëë.ðàáîòà z    ";
	ptrs[11]=		" T ïðîâåðêè   öåïè  ";
     ptrs[12]=		" áàòàðåè     qìèí.  ";
     ptrs[13]=		" Umax=       !Â     ";
     ptrs[14]=		" Umin=       ZÂ     ";
     ptrs[15]=		" Uá0°=       @Â     ";
     ptrs[16]=		" Uá20°=      #Â     ";
	...
	ptrs[26]=		" tè.max=     $°C    ";
	ptrs[27]=		" tè.ñèãí=    z°C    ";
	ptrs[28]=		" táàò.max=   b°C    ";
	ptrs[29]=		" táàò.ñèãí=  X°C    ";
     ptrs[30]=		" Âíåøíèå äàò÷èêè    ";
	ptrs[31]=		" Ethernet           ";
     ptrs[32]=      " Àäðåñ ñ÷åò÷èêà    +";
     ptrs[33]=      " Êîíòðîëü ñð.òî÷êè  ";
     ptrs[34]=      " áàòàðåè         Q% ";
	ptrs[35]=      " Ñåðèéíûé N        w";
     ptrs[36]=		" Âûõîä              ";
     ptrs[37]=		" Êàëèáðîâêè         "; 
     ptrs[38]=		"                    ";        
	...
	if(a_ind . i_s<19)
	     {
	     if(ZV_ON)sub_bgnd("ÂÊË.",'(',0);
	     else sub_bgnd("ÂÛÊ.",'(',0);
	     if(AV_OFF_AVT)sub_bgnd("àâòîì.",')',0);
	     else sub_bgnd("ðó÷í.",')',0);


	     if(MNEMO_ON==mnON)
	     	{
	     	sub_bgnd("÷åðåç yñ.",'y',-8);
	     	int2lcd(MNEMO_TIME,'y',0);
	     	}
	     else sub_bgnd("âûêë.",'y',-4);
	     int2lcd(UMAX,'!',1);
	...
	ptrs[0]=sm_time;
	ptrs[1]=sm_;
	if(phase==0)ptrs[2]="     <> - âûáîð     ";
     if(phase==1)ptrs[2]="   ^v - óñòàíîâêà   ";
     if(phase==2)ptrs[2]="     ¤  - âûõîä     ";
	
	bgnd_par(" ÓÑÒÀÍÎÂÊÀ  ÂÐÅÌÅÍÈ ",ptrs[0],ptrs[1],ptrs[2]);
...
else if((a_ind . i==iSet_T_ret)||(a_ind . i==iAir_sensor_kontr_ret)||(a_ind . i==iWater_sensor_kontr_ret))
	{
	...
	ptrs[1]=" Èñòî÷íèêîâ        !";
	ptrs[2]=" Èíâåðòîðîâ        ^";	
	ptrs[3]=" Êîíòðîëèðóåìûõ     ";
	ptrs[4]=" àâòîìàòîâ         $";
	ptrs[5]=" Âûõîä              ";

	if(a_ind . s_i<a_ind . i_s) a_ind . i_s=a_ind . s_i;
	else if((a_ind . s_i-a_ind . i_s)>2) a_ind . i_s=a_ind . s_i-2;
	bgnd_par("      ÑÒÐÓÊÒÓÐÀ     ",ptrs[a_ind . i_s],ptrs[a_ind . i_s+1],ptrs[a_ind . i_s+2]);
	...
	if(a_ind . s_i<a_ind . i_s) a_ind . i_s=a_ind . s_i;
     ...
     bgnd_par("ÄÀÒ×ÈÊ ÏÅÐÅÂÎÐÀ×ÈÂ. ",ptrs[a_ind . i_s],ptrs[a_ind . i_s+1],ptrs[a_ind . i_s+2]);
	
	pointer_set(1);

	if(sk_stat[3]==ssON)sub_bgnd("çàìêíóò",'@',0);
	else sub_bgnd("ðàçîìê.",'@',0);
	if(!SK_SIGN[3])sub_bgnd("çàìêíóò",'#',0);
	else sub_bgnd("íåçàìê.",'#',0);
	if(!SK_REL_EN[3])sub_bgnd("âêë.",'[',-2);
	else sub_bgnd("âûêë.",'[',-2);
	if(!SK_LCD_EN[3])sub_bgnd("âêë.",'(',-2);
	else sub_bgnd("âûêë.",'(',-2);
	...
	bgnd_par("   ÊÀËÈÁÐÎÂÊÀ ÑÅÒÈ  ",ptrs[a_ind . i_s],ptrs[a_ind . i_s+1],ptrs[a_ind . i_s+2]);
	...
	ptrs[0]=						" Áàòàðåÿ N1         ";
     ptrs[1]=						" Áàòàðåÿ N2         ";
     if(BAT_IS_ON[0]!=bisON)ptrs[0]=	" Áàòàðåÿ N2         ";
	ptrs[0+NUMBAT]=				" Âûõîä              ";
	ptrs[1+NUMBAT]=				"                    ";
	ptrs[2+NUMBAT]=				"                    ";
	...
	bgnd_par(" ÊÀËÈÁÐÎÂÊÀ ÁÀÒÀÐÅÉ ",ptrs[a_ind . i_s],ptrs[a_ind . i_s+1],ptrs[a_ind . i_s+2]);
	pointer_set(1);
...
#line 2725 "main.c"
    ...
    ptrs[19]=						" ÁÏÑ N12            ";               
	ptrs[8+NUMIST]=					" Âûõîä              ";
	ptrs[9+NUMIST]=					" Ïðîâåðêà WDT(âíóòð)";
	ptrs[10+NUMIST]=				" Ïðîâåðêà WDT(âíåøí)";

	if((a_ind . s_i-a_ind . i_s)>2)a_ind . i_s=a_ind . s_i-2;
	else if(a_ind . s_i<a_ind . i_s)a_ind . i_s=a_ind . s_i;
	bgnd_par("        ÒÅÑÒ        ",ptrs[a_ind . i_s],ptrs[a_ind . i_s+1],ptrs[a_ind . i_s+2]);
	pointer_set(1);
	...
	if(tst_state[0]==tst1) sub_bgnd("ÂÊË.",'!',-3);
	if(tst_state[0]==tst2) sub_bgnd("ÂÛÊË.",'!',-4);
	else sub_bgnd("ÐÀÁÎ×.",'!',-5);

	if(tst_state[1]==tst1) sub_bgnd("ÂÊË.",'@',-3);
	if(tst_state[1]==tst2) sub_bgnd("ÂÛÊË.",'@',-4);
	else sub_bgnd("ÐÀÁÎ×.",'@',-5);

	if(tst_state[2]==tst1) sub_bgnd("ÂÊË.",'#',-3);
	if(tst_state[2]==tst2) sub_bgnd("ÂÛÊË.",'#',-4);
	else sub_bgnd("ÐÀÁÎ×.",'#',-5);

	if(tst_state[3]==tst1) sub_bgnd("ÂÊË.",'$',-3);
	if(tst_state[3]==tst2) sub_bgnd("ÂÛÊË.",'$',-4);
	else sub_bgnd("ÐÀÁÎ×.",'$',-5);
	...
	bgnd_par(				" Îòêëþ÷åíèå Í.Ï.Í.  ",
							ptrs[a_ind . i_s],
							ptrs[a_ind . i_s+1],
							ptrs[a_ind . i_s+2]);
	pointer_set(1);
	
	if(NPN_OUT==npnoRELEVENT) sub_bgnd("Ðåëå âåíò-ðà",'!',-8);
	else if(NPN_OUT==npnoRELEAVBAT2) sub_bgnd("Ðåëå ÀÂ.ÁÀÒ2",'!',-8);
	else sub_bgnd("Âûêë.",'!',-1);
	...
	if(bFL_)
		{
		ptrs[1]=" óñòàíîâèòå Uàâòîí  ";
     	ptrs[2]="  íàæàòèåì š èëè œ  ";
     	}
     else 
     	{
		ptrs[1]=" óäåðæèâàéòå ¤ äëÿ  ";
     	ptrs[2]="    çàïîìèíàíèÿ     ";     	
     	}	
	...
	bgnd_par("       ÁÏÑ N!      ",ptrs[a_ind . i_s],ptrs[a_ind . i_s+1],ptrs[a_ind . i_s+2]);
	...
	if(DOP_RELE_FUNC==0)
		{
		ptrs[0]=	" Èíäèêàöèÿ          ";
		ptrs[1]=	" óñêîðåííîãî çàðÿäà ";
		}
	else  
		{
		ptrs[0]=	" Èíäèêàöèÿ          ";
		ptrs[1]=	" ðàçðÿæåííîé áàòàðåè";
		}
	ptrs[2]=		" Âûõîä              ";
	ptrs[3]=		"                    ";
